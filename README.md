...
In C++ they are `elastiqp::das::Solver`, `elastiqp::pdal::Solver` and `elastiqp::ipm::Solver` (one self-contained header each; `elastiqp::Solver` is the active-set default). A fourth, `elastiqp::sparse_pdal::Solver` (`include/elastiqp/sparse_pdal.hpp`), is the PDAL method on `Eigen::SparseMatrix` data for multiple-shooting MPC and other block-sparse problems: it factors the quasidefinite augmented KKT system with a sparse LDL' (pattern analyzed once), absorbs active-set changes with a Woodbury correction of the cached factor, and keeps its factorization across warm-started ticks (C++ only; see `docs/sparse_pdal.md`). In Python, `elastiqp.Solver(method)` / `elastiqp.solve(..., method=)` with `method` in `"das"`, `"pdal"`, `"ipm"`.
