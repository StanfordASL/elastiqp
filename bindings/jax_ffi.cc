...
//
// pdal: (Q, q, A, b, G, h, penalty) -> (x, t, y, z_t, z_ineq, info)
//
// ipm: (Q, q, A, b, G, h, penalty)
//       -> (x, t, y, z_t, z_ineq, xr, tr, yr, z_t_r, z_ineq_r, info)
//
// `ruiz` (0/1) enables equilibration on either backend
//
// info = [converged, iters] for pdal, [converged, iters, relax_converged]
// for ipm; converged/iters describe the tight solve, relax_converged the
// kappa relaxation (== converged when no relaxation runs). 
//
...
//
// For IPM, the (xr, ...) block is the kappa-relaxed central point used for
// smoothed implicit differentiation (see qpax): when target_kappa > 0, we
// re-solve from the optimum to the point satisfying the same KKT conditions
// with complementarity s.z = kappa, and the Python wrapper differentiates
// there while still returning the tight solution as the value. When
// target_kappa <= 0 the relaxed block is a copy of the tight solution
// (exact gradients).
//
...
ffi::Error ElastiqpIpmImpl(
  ...
  const elastiqp::Solution& rsol =
      (target_kappa > 0 && p > 0)
          ? solver.relax(target_kappa, eps_abs, static_cast<int>(max_iter))
          : sol;
...
ffi::Error ElastiqpPdalImpl(
    double eps_abs, int64_t max_iter, int64_t ruiz, ffi::Buffer<ffi::F64> Q,
    ffi::Buffer<ffi::F64> q, ffi::Buffer<ffi::F64> A, ffi::Buffer<ffi::F64> b,
    ffi::Buffer<ffi::F64> G, ffi::Buffer<ffi::F64> h,
    ffi::Buffer<ffi::F64> penalty, ffi::ResultBuffer<ffi::F64> x,
    ffi::ResultBuffer<ffi::F64> t, ffi::ResultBuffer<ffi::F64> y,
    ffi::ResultBuffer<ffi::F64> z_t, ffi::ResultBuffer<ffi::F64> z_ineq,
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(ElastiqpIpmSolve, ElastiqpIpmImpl,
                              ffi::Ffi::Bind()
                                  .Attr<double>("eps_abs")
                                  .Attr<int64_t>("max_iter")
                                  .Attr<int64_t>("ruiz")
