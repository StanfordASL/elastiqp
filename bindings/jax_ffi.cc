...
// complementarity s.z = kappa, and the Python wrapper differentiates there
// while still returning the tight solution as the value. When
// target_kappa <= 0 the relaxed block is a copy of the tight solution.
// Both backends target the same central point (the IPM by walking the
// central path, the PDAL through the log-barrier retraction), so their
// smoothed gradients agree.
//
